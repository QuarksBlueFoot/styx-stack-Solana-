...
import nexus.styx.core.SpsInstructionInfo
import nexus.styx.core.SpsDomains as Domains
        ...
        return when {
            Domains.isCompact(domain) && data.size >= 2 -> {
                ...
                val opName = when (domain) {
                    ...
                    Domains.MESSAGING -> MessagingOps.name(op)
                    Domains.ACCOUNT -> AccountOps.name(op)
                    Domains.VSL -> VslOps.name(op)
