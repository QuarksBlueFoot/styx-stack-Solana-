 ...
 * 
 * This is the bridge between X3DH key agreement and Double Ratchet messaging.
 * Called after x3dhInitiate() or x3dhRespond() completes.
 * 
 * @param x3dhSharedSecret The shared secret from X3DH key agreement
 * @param ourEphemeralPublic Our ephemeral public key from X3DH
 * @param theirIdentityPublic Their identity public key (for session binding)
 * @param isInitiator True if we initiated the X3DH exchange
 * @returns RatchetState ready for messaging
 */
export function initRatchetFromX3DH(
  x3dhSharedSecret: Uint8Array,
  ourEphemeralPublic: Uint8Array,
  theirIdentityPublic: Uint8Array,
  isInitiator: boolean
): RatchetState {
  // Use X3DH shared secret directly as the initial shared secret
  // The initRatchet function handles the key derivation
  return initRatchet(
    x3dhSharedSecret,
    ourEphemeralPublic,
    theirIdentityPublic,
